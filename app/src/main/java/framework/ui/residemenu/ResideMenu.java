package framework.ui.residemenu;
...
public class ResideMenu extends FrameLayout{
    ...
    private float mScaleValueY = 0.5f;
    ...
    private void initViews(Context context){
        ...
        inflater.inflate(R.layout.ui_residemenu, this);
     ...
     * you can modification the animation
     */
    public void openMenu(int direction){
        ...
        AnimatorSet scaleDown_activity = buildScaleDownAnimation(viewActivity,
                mScaleValueX, mScaleValueY);
        Log.d("My_Tag", "shadowAdjustScaleX = " + shadowAdjustScaleX + "\tshadowAdjustScaleY" + shadowAdjustScaleY);
     ...
     *
     * @param target
     * @param targetScaleX targetScaleX越大,离右边距越近
     * @param targetScaleY targetScaleY越大,离上边距越近
     * @return
     */
    private AnimatorSet buildScaleDownAnimation(View target,float targetScaleX,float targetScaleY){
        Log.d("My_Tag","buildScaleDownAnimation scaleX = " + targetScaleX + "\tscaleY" + targetScaleY);
    ...
    private AnimatorSet buildScaleUpAnimation(View target,float targetScaleX,float targetScaleY){
        ...
        Log.d("My_Tag","buildScaleUpAnimation scaleX = " + targetScaleX + "\tscaleY" + targetScaleY);
        scaleUp.setDuration(250);
        return scaleUp;
    }

    private AnimatorSet buildMenuAnimation(View target, float alpha){
        Log.d("My_Tag", "alpha = " + alpha);
        AnimatorSet alphaAnimation = new AnimatorSet();
        if (scaleDirection == DIRECTION_RIGHT) {
            alphaAnimation.playTogether(
                    ObjectAnimator.ofFloat(target, "alpha", alpha)
                    //ObjectAnimator.ofFloat(target, "translationX", -100)
            );
        }else {
            alphaAnimation.playTogether(
                    ObjectAnimator.ofFloat(target, "alpha", alpha)
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        Log.d("dispatchTouchEvent", "currentActivityScaleX = " + currentActivityScaleX);
        ...
        switch (ev.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                /**
                 * xOffset X方向的偏移量
                 * lastActionDownX 上一次按下的X坐标
                 * */
                int xOffset = (int) (ev.getX() - lastActionDownX);
                /**
                 * Y方向的偏移量
                 * lastActionDownY 上一次按下的Y坐标
                 * */
                ...
                if(pressedState == PRESSED_DOWN) {
                    ...
                    if(xOffset < -50 || xOffset > 50) {//手指往右边滑动xOffset为正数
                        pressedState = PRESSED_MOVE_HORIZANTAL;
                    ...
                    //内容的缩放
                    Log.d("dispatchTouchEvent", "targetScale = " + targetScale);
                    float maxScale = (mScaleValueX > mScaleValueY) ? mScaleValueX : mScaleValueY;

                    ViewHelper.setScaleX(viewActivity, targetScale);
                    ViewHelper.setScaleY(viewActivity, (maxScale > targetScale) ? maxScale : targetScale);
                    ViewHelper.setScaleX(imageViewShadow, targetScale + shadowAdjustScaleX);
                    ViewHelper.setScaleY(imageViewShadow, (maxScale > targetScale) ? maxScale : targetScale + shadowAdjustScaleY);
            ...
            case MotionEvent.ACTION_UP:
                ...
                /**
                 * 在menu打开的条件之下,如果currentActivityScaleX的缩放比例大于0.56(数值越大,离右边距越近),则关闭menu;否则打开
                 * */
                if (isOpened()){
    ...
    public void setScaleValueX(float scaleValue) {
        this.mScaleValueX = scaleValue;
    }

    public void setScaleValueY(float scaleValue) {
        this.mScaleValueY = scaleValue;
    }
